...
guess_encoding("13112_Setagaya-ku09.csv") # ""内に確認するファイル名を入力
...
site.n <- import("13112_Setagaya-ku09.csv", setclass= "tbl_df", encoding ="UTF-8")
...
site.coord <- import("13112_Setagaya-ku09_coord.csv", setclass= "tbl_df", encoding ="UTF-8")
...
site.subtotal <- import("13112_Setagaya-ku09.csv", setclass= "tbl_df", encoding ="CP932")
  # site.nにBBB.csvをテーブルとして(setclass = "tbl_df")読み込み
  # site.n = 読み込み先のオブジェクト名(作業用)
  # BBB = 読み込み元のファイル名(ローカルリポジトリ内なのでファイル名のみ)
...
#不要な列の削除
  site.subtotal <- select(site.subtotal,-色,-取得住所) #ひなたGISでデフォルトで付加される列を削除
...
# 多い場合の処理
 # site.subtotal <- select(site.subtotal,-"取得住所") 
```
...
#site.totalにも追加
site.total <- left_join(site.total,select(site.total.age,-自治体コード, -遺跡名, -緯度, -経度),by = "JASID")
...
if (exists("site.total.age") ==FALSE) {
  site.total <- import("13Tokyo_totalAge.csv",setclass = "tbl_df", encoding = "UTF-8")
}
